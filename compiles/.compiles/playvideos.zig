...
pub fn setsid() !linux.pid_t {
    // with the help of Lee Cannon on discord
    const rc = linux.syscall0(.setsid);
    switch (linux.getErrno(rc)) {
        .SUCCESS => return @intCast(linux.pid_t, rc),
...
const Player = struct {
    ...
    const Pids = std.TailQueue(os.pid_t);
    const Self = @This();

    fn spawn(allocator: mem.Allocator, argv: []const []const u8) !os.pid_t {
    ...
    fn init(allocator: mem.Allocator, parallel: u4, toc: TOC) Self {
        return .{
            .parallel = parallel,
            .allocator = allocator,
            .toc = toc,
    ...
    fn deinit(self: *Self) void {
        ...
        while (next) |node| {
            next = node.next;
            const pid = node.data;
            os.kill(pid, linux.SIG.HUP) catch |err| {
    ...
    fn play(self: *Self) !void {
        ...
        while (true) {
            ...
            const pid = try spawn(self.allocator, &.{ "/usr/bin/mpv", "--mute=yes", "--no-terminal", file });
    ...
    fn makeOneRoom(self: *Self) void {
        const dead = os.waitpid(-1, linux.W.NOHANG);
        var next = self.pids.first;
        var found: u4 = 0;
        while (next) |node| {
            next = node.next;
...
const act_chld: linux.Sigaction = .{
...
const act_exit: linux.Sigaction = .{
...
pub fn main() !void {
    ...
    const roots = blk: {
        ...
        break :blk arg_roots.toOwnedSlice();
    };
    ...
    var player = Player.init(allocator, 4, toc);
    defer player.deinit();
    ...
    try os.sigaction(linux.SIG.INT, &act_exit, null);
    try os.sigaction(linux.SIG.TERM, &act_exit, null);
...
// # asyncrun: zig build-exe -O ReleaseFast -fsingle-threaded
