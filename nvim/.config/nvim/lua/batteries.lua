...
local facts = (function(native_rtps)
  ...
  do
    ...
    base.plugin_rtps.guwen = { pre = pathto("hybrids/guwen") }
    base.plugin_rtps.cricket = { pre = pathto("hybrids/cricket") }
    base.plugin_rtps.sh = { pre = pathto("hybrids/sh") }
  end
  do
    ...
    for _, val in ipairs(files) do
      table.insert(base.builtin_plugins, root .. "/" .. val)
    end
  end
  return base
end)(vim.go.rtp)
...
local profile_plugins = {
  { "base", { "cthulhu", "natives", "sh", "wiki" } },
  { "joy", { "guwen", "cricket" } },
  { "code", { "easy-align", "surround" } },
}
...
local function source_file(fpath)
  local ok, err = xpcall(api.nvim_cmd, debug.traceback, { cmd = "source", args = { fpath } }, {})
