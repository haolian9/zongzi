...
do --coreutils
  do
    ...
    local spell = cmds.Spell("Edit", function(args)
      ...
      require("puff").input({ prompt = "edit" }, function(name)
  ...
  do --:Mv
    local spell = cmds.Spell("Mv", function(args)
      ...
      require("puff").input({ prompt = "mv", default = default }, function(name)
        if name == nil or name == "" then return end
  ...
  do
    ...
    local spell = cmds.Spell("Mkdir", function(args)
      ...
      require("puff").input({ prompt = "mkdir" }, function(name)
        if name == nil or name == "" then return end
        main(name)
      end)
    end)
...
do --grep
  ...
  do --:Grep
    local spell = cmds.Spell("Grep", function(args)
      local grep = require("grep")
      if args.regex then
        grep.text(args.regex)
      else
        grep.input()
      end
    end)
  ...
  do --:Rg
    ...
    local spell = cmds.Spell("Rg", function(args)
      local extra = {}
      local iter = itertools.filtern(is_extra_flag, dictlib.items(args))
...
do --bufgrep
  m.x("<leader>g", [[:lua require"bufgrep".vsel()<cr>]])
...
do --beckon
  ...
  do
    local spell = cmds.Spell("Beckon", function(args) assert(require("beckon")[args.provider])() end)
    spell:add_arg("provider", "string", true, nil, cmds.ArgComp.constant({ "digraphs", "emojis" }))
    cmds.cast(spell)
  end
end
...
do --:Git
  ...
  local handlers = {
    ...
    push = function() require("digits.cmds.push").floatwin() end,
    hunks = function() require("digits.cmds.diffhunks").setloclist() end,
    diff = function() require("digits.cmds.diff")() end,
    diff_file = function() require("digits.cmds.diff")(nil, ni.get_current_buf()) end,
    ...
    fixup = function() require("digits.cmds.fixup").floatwin() end,
  }
