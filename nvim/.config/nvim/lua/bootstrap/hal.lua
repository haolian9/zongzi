...
do --status&tabline
  prefer.def.statusline = [[%!v:lua.require'linesexpr.statusline'.render()]]
  prefer.def.tabline = [[%!v:lua.require'linesexpr.tabline'.render()]]
...
do --re-define builtin keymap
  ...
  m.n(".", function() repeats.rhs_dot(ni.get_current_buf()) end)
...
do --coreutils
  ...
  cmds.create("Rm", function() coreutils.rm_filebuf(0) end)
...
do --eureka
  ...
  cmds.create("Todo", function() require("eureka").text([[\btodo\b]]) end)
...
do --:Expose
  local function try_expose(a, b)
    ...
    _G[b] = mod
    jelly.info("module %s exposed as %s", a, b)
    return true
  end

  cmds.create("Expose", function(args)
    local pairs = args.fargs
    if #pairs == 1 then pairs[2] = pairs[1] end
    ...
    for a in iter do
      ...
      jelly.info("module %s not found", a)
      ::continue::
    end
  end, { nargs = "+" })
end
...
do --misc cmds
  ...
  cmds.create("BufOnly", function()
    --todo: nvim resources: treesit
    --todo: external resources: lsp, cricket/mpv
    --todo: different buf: buftype, listed, loaded,
    ex.eval("silent 1,-.bw")
    ex.eval("silent .+,$bw")
  end)
end
