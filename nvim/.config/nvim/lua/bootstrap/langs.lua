...
do
  function general_lsp.start(langser, bufnr)
    local spec = assert(lsp.config[langser])
  ...
  do
    ...
    local function make_onlist_fn(open_mode)
      ...
      return function(args)
        ...
        --todo: deduplicate
        local loclist = sting.location.shelf(winid, args.context.method)
        loclist:reset()
...
do
  ...
  add("python", {
    ...
    lsp = function(bufnr) --
      general_lsp.start("pyright", bufnr)
      general_lsp.prefers(bufnr)
      general_lsp.keymaps(bufnr)
    end,
  })
...
local winspecs = {
  ...
  make = { vim = function(bufnr, winid) prefer.wo(winid, "list", false) end },
  ...
  glsl = {
    treesitter = function(bufnr, winid) require("squirrel.folding").attach(winid, "glsl") end,
  },
}
