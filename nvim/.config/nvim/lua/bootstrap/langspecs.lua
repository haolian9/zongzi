  ...
  adds({
    lua = {
      vim = function(bufnr)
        ...
        bm.n("<leader>r", function() require("windmill").run() end)
    ...
    cmake = {
      ...
      lsp = function(bufnr) vim.lsp.start(langserspecs("cmakels", resolve_root_dir(), profiles.has("powersave")), { bufnr = bufnr }) end,
    },
  })
  -- misc
  adds({
...
do -- main
  local aug = augroups.Augroup("boot://langspecs")
