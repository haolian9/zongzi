---changes:
---* simplified Client:on_attach
---  * no use of lsp._set_defaults
...
local function validate_encoding(encoding)
  ...
  if not encoding then return valid_encodings.utf16 end
  return valid_encodings[encoding:lower()] or error(string.format("Invalid position encoding %q. Must be one of: 'utf-8', 'utf-16', 'utf-32'", encoding))
end
...
local function optional_validator(fn)
  return function(v) return v == nil or fn(v) end
end
...
local function default_get_language_id(_bufnr, filetype) return filetype end
...
local function validate_config(config)
  ...
  assert((not config.flags or not config.flags.debounce_text_changes or type(config.flags.debounce_text_changes) == "number"), "flags.debounce_text_changes must be a number with the debounce time in milliseconds")
end
...
local function get_name(id, config)
  ...
  if type(config.cmd) == "table" and config.cmd[1] then return assert(vim.fs.basename(config.cmd[1])) end
...
function Client.create(config)
  ...
  self.capabilities = vim.tbl_deep_extend("force", lsp.protocol.make_client_capabilities(), self.capabilities or {})
  ...
  self.dynamic_capabilities = {
    ...
    client_id = id,
    register = function(_, registrations) return self:_register_dynamic(registrations) end,
    unregister = function(_, unregistrations) return self:_unregister_dynamic(unregistrations) end,
    get = function(_, method, opts) return self:_get_registration(method, opts and opts.bufnr) end,
    supports_registration = function(_, method) return self:_supports_registration(method) end,
    supports = function(_, method, opts) return self:_get_registration(method, opts and opts.bufnr) ~= nil end,
  }
  ...
  local dispatchers = {
    notification = function(...) return self:_notification(...) end,
    server_request = function(...) return self:_server_request(...) end,
    on_error = function(...) return self:_on_error(...) end,
    on_exit = function(...) return self:_on_exit(...) end,
  }
...
function Client:_run_callbacks(cbs, error_id, ...)
  for _, cb in pairs(cbs) do
    ...
    if not status then self:write_error(error_id, err) end
  end
end
...
function Client:initialize()
  ...
  self:_run_callbacks({ self._before_init_cb }, lsp.client_errors.BEFORE_INIT_CALLBACK_ERROR, init_params, config)
  ...
  rpc.request("initialize", init_params, function(init_err, result)
    ...
    self.server_capabilities = assert(result.capabilities, "initialize result doesn't contain capabilities")
    self.server_capabilities = assert(lsp.protocol.resolve_capabilities(self.server_capabilities))

    if self.server_capabilities.positionEncoding then self.offset_encoding = self.server_capabilities.positionEncoding end

    self.server_info = result.serverInfo

    if next(self.settings) then self:notify(ms.workspace_didChangeConfiguration, { settings = self.settings }) end
    ...
    for buf in pairs(reattach_bufs) do
      ...
      if self.attached_buffers[buf] then self:on_attach(buf) end
    end

    log.info(self._log_prefix, "server_capabilities", { server_capabilities = self.server_capabilities })
  end)
end
...
function Client:_resolve_handler(method) return self.handlers[method] or lsp.handlers[method] end
...
function Client:_process_request(id, req_type, bufnr, method)
  ...
  if pending and cur_request then
    log.error(self._log_prefix, ("Cannot create request with id %d as one already exists"):format(id))
    return
  elseif not pending and not cur_request then
    log.error(self._log_prefix, ("Cannot find request with id %d whilst attempting to %s"):format(id, req_type))
    return
  end
...
function Client:request(method, params, handler, bufnr)
  if not handler then handler = assert(self:_resolve_handler(method), string.format("not found: %q request handler for client %q.", method, self.name)) end
  ...
  local success, request_id = self.rpc.request(
    method,
    params,
    function(err, result)
      handler(err, result, {
    ...
    function(request_id)
      ...
      if request_registered then self:_process_request(request_id, "complete") end
...
function Client:request_sync(method, params, timeout_ms, bufnr)
  ...
  local function _sync_handler(err, result) request_result = { err = err, result = result } end

  local success, request_id = self:request(method, params, _sync_handler, bufnr)
  if not success then return nil end

  local wait_result, reason = vim.wait(timeout_ms or 1000, function() return request_result ~= nil end, 10)

  if not wait_result then
    if request_id then self:cancel_request(request_id) end
...
function Client:notify(method, params)
  if method ~= ms.textDocument_didChange then changetracking.flush(self) end

  local client_active = self.rpc.notify(method, params)

  if client_active then vim.schedule(function()
...
function Client:stop(force)
  if type(force) == "number" then vim.defer_fn(function() self:stop(true) end, force) end

  local rpc = self.rpc
  if rpc.is_closing() then return end
...
function Client:_register_dynamic(registrations)
  ...
  for _, reg in ipairs(registrations) do
    ...
    if not self.registrations[method] then self.registrations[method] = {} end
...
function Client:_register(registrations)
  ...
  if #unsupported > 0 then
    local warning_tpl = "The language server %s triggers a registerCapability " .. "handler for %s despite dynamicRegistration set to false. " .. "Report upstream, this warning is harmless"
...
function Client:_unregister(unregistrations)
  ...
  for _, unreg in ipairs(unregistrations) do
    if unreg.method == ms.workspace_didChangeWatchedFiles then vim.lsp._watchfiles.unregister(unreg, self.id) end
  end
end

--- @private
function Client:_get_language_id(bufnr) return self.get_language_id(bufnr, vim.bo[bufnr].filetype) end
...
function Client:_get_registration(method, bufnr)
  ...
  for _, reg in ipairs(self.registrations[method] or {}) do
    ...
    if not regoptions or not regoptions.documentSelector then return reg end
    ...
    for _, filter in ipairs(documentSelector) do
      ...
      if not (flang and language ~= flang) and not (fscheme and not vim.startswith(uri, fscheme .. ":")) and not (type(fpat) == "string" and not vim.glob.to_lpeg(fpat):match(fname)) then return reg end
    end
  end
end
...
function Client:is_stopped() return self.rpc.is_closing() or self._is_stopping end
...
function Client:exec_cmd(command, context, handler)
  ...
  if not vim.list_contains(commands, cmdname) then
    vim.notify_once(string.format("Language server `%s` does not support command `%s`. This command may require a client extension.", self.name, cmdname), vim.log.levels.WARN)
    return
  end
...
function Client:_text_document_did_open_handler(bufnr)
  ...
  if not self:supports_method(ms.textDocument_didOpen) then return end
...
function Client:on_attach(bufnr)
  ...
  do -- simplified lsp._set_defaults
    if self:supports_method(ms.textDocument_diagnostic, bufnr) then --
...
function Client:supports_method(method, bufnr)
  ...
  if not required_capability then return true end
  if vim.tbl_get(self.server_capabilities, unpack(required_capability)) then return true end
  ...
  if rmethod then
    ...
    if self:_supports_registration(method) then return self:_get_registration(method, bufnr) ~= nil end
  end
  return false
end
...
function Client:_get_registration_options(method, bufnr)
  if not self:_supports_registration(method) then return end

  local reg = self:_get_registration(method, bufnr)

  if reg then return reg.registerOptions end
end
...
function Client:_on_exit(code, signal) self:_run_callbacks(self._on_exit_cbs, lsp.client_errors.ON_EXIT_CALLBACK_ERROR, code, signal, self.id) end
...
function Client:_add_workspace_folder(dir)
  ...
  if not self.workspace_folders then self.workspace_folders = {} end
