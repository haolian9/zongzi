  ...
  for k, pattern_list in pairs(self.info.patterns) do
    ...
    for _, pattern in ipairs(pattern_list) do
      ...
      if is_directive(pred_name) then
        ...
        if vim.deep_equal(pattern, { "set!", "injection.combined" }) then self.has_combined_injections = true end
        if vim.deep_equal(pattern, { "set!", "conceal_lines", "" }) then self.has_conceal_line = true end
      else
...
M.parse = memoize("concat-2", function(lang, query)
  local added, err = language.add(lang)
  if not added then error(err) end
...
local predicate_handlers = {
  ["eq?"] = function(match, _, source, predicate) return impl["eq"](match, source, predicate, false) end,

  ["any-eq?"] = function(match, _, source, predicate) return impl["eq"](match, source, predicate, true) end,

  ["lua-match?"] = function(match, _, source, predicate) return impl["lua-match"](match, source, predicate, false) end,

  ["any-lua-match?"] = function(match, _, source, predicate) return impl["lua-match"](match, source, predicate, true) end,

  ["match?"] = function(match, _, source, predicate) return impl["match"](match, source, predicate, false) end,

  ["any-match?"] = function(match, _, source, predicate) return impl["match"](match, source, predicate, true) end,

  ["contains?"] = function(match, _, source, predicate) return impl["contains"](match, source, predicate, false) end,

  ["any-contains?"] = function(match, _, source, predicate) return impl["contains"](match, source, predicate, true) end,
  ...
  ["any-of?"] = function(match, _, source, predicate)
    ...
    for _, node in ipairs(nodes) do
      ...
      if string_set[node_text] then return true end
    end
  ...
  ["has-ancestor?"] = function(match, _, _, predicate)
    ...
    for _, node in ipairs(nodes) do
      if node:__has_ancestor(predicate) then return true end
    end
    return false
  end,
  ...
  ["has-parent?"] = function(match, _, _, predicate)
    ...
    for _, node in ipairs(nodes) do
      if vim.list_contains({ unpack(predicate, 3) }, node:parent():type()) then return true end
    end
    return false
  end,
}
...
function M.add_predicate(name, handler, opts)
  ...
  if predicate_handlers[name] and not opts.force then error(string.format("Overriding existing predicate %s", name)) end
  ...
  if opts.all ~= false then
    ...
    local function wrapper(match, ...)
      ...
      for k, v in pairs(match) do
        if type(k) == "number" then m[k] = v[#v] end
      end
      return handler(m, ...)
    end
...
function M.add_directive(name, handler, opts)
  ...
  if directive_handlers[name] and not opts.force then error(string.format("Overriding existing directive %s", name)) end
...
function Query:_match_predicates(predicates, pattern_i, captures, source)
  for _, predicate in ipairs(predicates) do
    ...
    if does_match ~= should_match then return false end
  end
  return true
end
...
function Query:_apply_directives(directives, pattern_i, captures, source)
  ...
  for _, directive in pairs(directives) do
    ...
    if not handler then error(string.format("No handler for %s", directive[1])) end
...
local function value_or_node_range(start, stop, node)
  if start == nil then start = node:start() end
  if stop == nil then
...
function Query:iter_captures(node, source, start, stop, opts)
  ...
  local function iter(end_line)
    ...
    --- @type vim.treesitter.query.TSMetadata
    local metadata
    if match_id <= highest_cached_match_id then metadata = match_cache[match_id] end
    ...
    if not metadata then
      ...
      if processed_pattern then
        ...
        if not self:_match_predicates(predicates, pattern_i, captures, source) then
          ...
          if end_line and captured_node:range() > end_line then return nil, captured_node, nil, nil end
...
--- @param findstart 0|1
--- @param base string
function M.omnifunc(findstart, base) return vim.treesitter._query_linter.omnifunc(findstart, base) end
...
function M.edit(lang) assert(vim.treesitter.dev.edit_query(lang)) end
