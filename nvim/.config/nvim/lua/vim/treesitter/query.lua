...
do
  ...
  function M.set(lang, query_name, text) explicit_queries[lang][query_name] = M.parse(lang, text) end
  ...
  local predicate_handlers = {
    ...
    ["has-ancestor?"] = function(match, _, _, predicate)
      ...
      for _, node in ipairs(nodes) do
        ...
        while cur do
          if ancestor_types[cur:type()] then return true end
          cur = cur:parent()
        end
      end
      return false
  ...
  function Query:match_preds(match, source)
    ...
    if not preds then return true end
  ...
  function Query:apply_directives(match, source)
    ...
    if not preds then return metadata end
    ...
    for _, pred in pairs(preds) do
      if is_directive(pred[1]) then
        ...
        if not handler then error(string.format("No handler for %s", pred[1])) end
  ...
  --- @param match TSQueryMatch
  --- @return integer
  local function match_id_hash(_, match) return (match:info()) end
...
function M.edit(lang) vim.treesitter.dev.edit_query(lang) end
