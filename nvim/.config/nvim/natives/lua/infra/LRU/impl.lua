...
---haoliang modified this source code to fit his need
---* no delete
---* not return dict
---* maybe remove the bytes feature

---@param max_size integer
---@param max_bytes? integer
return function(max_size, max_bytes)
  ...
  local function set(_, key, value, bytes)
    local tuple = map[key]
    if tuple then del(key, tuple) end
    if value ~= nil then
  ...
  local function lru_pairs() return mynext, nil, nil end

  return get, set, lru_pairs
end
