...
local jelly = require("infra.jellyfish")("statusline")
...
local resolve_bufname = require("linesexpr.resolve_bufname")

local log = logging.newlogger("linesexpr.statusline", "debug")
...
do
  ...
  local function resolve_fname(bufnr)
    ...
    if bufname == "" then
      return resolve_bufname.unnamed.filetype(bufnr, bufname) or resolve_bufname.unnamed.number(bufnr, bufname)
    else
      return resolve_bufname.named.protocol(bufnr, bufname) or resolve_bufname.named.relative_stem(bufnr, bufname)
    end
  end
  ...
  local function resolve_alt_fname(alt_bufnr)
    ...
    if bufname == "" then
      return resolve_bufname.unnamed.short_filetype(alt_bufnr, bufname) or resolve_bufname.unnamed.number(alt_bufnr, bufname)
    else
      return resolve_bufname.named.short_protocol(alt_bufnr, bufname) or resolve_bufname.named.stem(alt_bufnr, bufname)
    end
  end
...
function M.render(fresh)
  ...
  local ok, result = pcall(build, fresh)
  if ok then return result end
  log.err("statusline build failed: %s", result)
  return "--xx--stl-err-see-log--xx--"
end
