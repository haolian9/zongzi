...
do
  ...
  local function resolve_tab_title(tabpage)
    ...
    if bufname == "" then
      return resolve_bufname.unnamed.short_filetype(bufnr, bufname) or resolve_bufname.unnamed.number(bufnr, bufname)
    else
      return resolve_bufname.named.short_protocol(bufnr, bufname) or resolve_bufname.named.relative_stem(bufnr, bufname)
    end
  end
