...
do
  ...
  function repeat_cmd()
    ...
    if cmd == nil or #cmd == 0 then return "" end
...
do
  ...
  local records = dictlib.CappedDict(256)
  --todo: group by severity?

  local aug = Augroup("statusline://")
...
do
  ...
  local function resolve_fname(bufnr)
    ...
    if bufname == "" then
      return bufdispname.unnamed.filetype(bufnr, bufname) or bufdispname.unnamed.number(bufnr, bufname)
    else
      return bufdispname.named.protocol(bufnr, bufname) or bufdispname.named.relative_stem(bufnr, bufname)
    end
  end
  ...
  local function resolve_alt_fname(alt_bufnr)
    ...
    local bufname = api.nvim_buf_get_name(alt_bufnr)
    if bufname == "" then
      return bufdispname.unnamed.short_filetype(alt_bufnr, bufname) or bufdispname.unnamed.number(alt_bufnr, bufname)
    else
      return bufdispname.named.short_protocol(alt_bufnr, bufname) or bufdispname.named.stem(alt_bufnr, bufname)
    end
  end
