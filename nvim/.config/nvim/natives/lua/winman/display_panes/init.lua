...
local Alphabet = require("winman.display_panes.Alphabet")
local g = require("winman.g")
...
do
  ...
  function build_matrix(exclude_winids)
    exclude_winids = exclude_winids or {}

    local excludes = itertools.toset(exclude_winids)
    local matrix = zero_matrix()

    for wi in iter_wi() do
      if not excludes[wi.winid] then --
        draw_win(matrix, wi)
      end
    end
...
---@param callback fun(winnr:integer)
---@param exclude_winids? integer[]
return function(callback, exclude_winids)
  ...
  do
    local lines = {}
    for i, line in ipairs(build_matrix(exclude_winids)) do
  ...
  do
    ...
    wo.list = false
    wo.winblend = 1
    ni.win_set_cursor(winid, { screenpos.row, screenpos.col })
  end
end
